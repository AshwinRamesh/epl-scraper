...
class DataPlayerFixture extends PlayerFixture {
    ...
    function __construct($dataArray, $player_id) {
        if ($dataArray) {
            $this->parseDataToObject($dataArray, $player_id);
        }
    }
    ...
    function parseDataToObject($data, $player_id) {
        try {
            ...
            $this->set_fixtureId($this->get_fixture_id($data[1], $data[0], $data[2]));
            ...
            $this->set_penaltiesSaved($data[9]);
            $this->set_penaltiesMissed($data[10]);
            $this->set_yellowCard($data[11]);
            $this->set_redCard($data[12]);
            $this->set_saves($data[13]);
            $this->set_bonus($data[14]);
            $this->set_eaSportsPPI($data[15]);
            $this->set_bonusPointSystem($data[16]);
            $this->set_netTransfers($data[17]);
            $this->set_costValue($data[18]);
            $this->set_points($data[19]);
        } catch (Exception $e) {
            echo "$e";
        }
    }
